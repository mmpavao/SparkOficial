...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/admin/bulk-import/users', requireAuth, requireAdmin, async (req: any, res) => {
    try {
      ...
      for (let i = 0; i < users.length; i++) {
        ...
        try {
          ...
          if (!userData.companyName || !userData.cnpj || !userData.fullName || 
              !userData.phone || !userData.email || !userData.password) {
            throw new Error(`Linha ${i + 1}: Campos obrigatórios faltando`);
          }

          // Check if user already exists
          const existingUser = await storage.getUserByEmail(userData.email);
          if (existingUser) {
            throw new Error(`Linha ${i + 1}: Email ${userData.email} já existe`);
          }
      ...
      res.json({
        message: `Importação concluída: ${results.success} usuários criados, ${results.errors.length} erros`,
        results
      });
  ...
  // Bulk import endpoint for credit applications
  app.post('/api/admin/bulk-import/credit-applications', requireAuth, requireAdmin, async (req: any, res) => {
    try {
      const { applications } = req.body;
      
      if (!Array.isArray(applications) || applications.length === 0) {
        return res.status(400).json({ message: "Lista de aplicações é obrigatória" });
      }
      ...
      for (let i = 0; i < applications.length; i++) {
        ...
        try {
          // Validate required fields
          if (!appData.userId || !appData.legalCompanyName || !appData.cnpj || 
              !appData.address || !appData.city || !appData.state || !appData.requestedAmount) {
            throw new Error(`Linha ${i + 1}: Campos obrigatórios faltando`);
          }

          // Verify user exists
          const user = await storage.getUser(appData.userId);
          if (!user) {
            throw new Error(`Linha ${i + 1}: Usuário ID ${appData.userId} não encontrado`);
          }
          ...
          const applicationData = {
            ...
            tradingName: appData.tradingName || null,
            cnpj: appData.cnpj,
            stateRegistration: appData.stateRegistration || null,
            municipalRegistration: appData.municipalRegistration || null,
            address: appData.address,
            city: appData.city,
            state: appData.state,
            zipCode: appData.zipCode || '00000-000',
            phone: appData.phone || user.phone,
            email: appData.email || user.email,
            website: appData.website || null,
            shareholders: appData.shareholders || [{"name": "Proprietário", "cpf": "000.000.000-00", "percentage": "100"}],
            businessSector: appData.businessSector || 'Comércio',
            annualRevenue: appData.annualRevenue || '500000-1000000',
            mainImportedProducts: appData.mainImportedProducts || 'Produtos diversos',
            mainOriginMarkets: appData.mainOriginMarkets || 'China',
            requestedAmount: appData.requestedAmount.toString(),
            currency: appData.currency || 'USD',
            productsToImport: appData.productsToImport || ['Produtos diversos'],
            monthlyImportVolume: appData.monthlyImportVolume || '50000',
            justification: appData.justification || 'Expansão de negócios',
            status: appData.status || 'pending',
            currentStep: 4,
            documentsStatus: 'pending',
            preAnalysisStatus: appData.preAnalysisStatus || 'pending',
            riskLevel: appData.riskLevel || 'medium',
            analysisNotes: appData.analysisNotes || null,
            requestedDocuments: appData.requestedDocuments || null,
            adminObservations: appData.adminObservations || null,
            financialStatus: appData.financialStatus || 'pending_financial',
            creditLimit: appData.creditLimit || null,
            approvedTerms: appData.approvedTerms || null,
            financialNotes: appData.financialNotes || null,
            adminStatus: appData.adminStatus || 'pending_admin',
            finalCreditLimit: appData.finalCreditLimit || null,
            finalApprovedTerms: appData.finalApprovedTerms || null,
            finalDownPayment: appData.finalDownPayment || '30',
            adminFee: appData.adminFee || '10'
          };

          // Create application
          const newApplication = await storage.createCreditApplication(applicationData);

          results.success++;
          results.created.push({
            id: newApplication.id,
            userId: appData.userId,
            companyName: appData.legalCompanyName,
          ...
          results.errors.push({
            line: i + 1,
            userId: appData.userId || 'N/A',
            companyName: appData.legalCompanyName || 'N/A',
            error: error.message
          });
        }
      }
      ...
      res.json({
        message: `Importação concluída: ${results.success} aplicações criadas, ${results.errors.length} erros`,
        results
      });

    } catch (error) {
      console.error("Bulk import credit applications error:", error);
      res.status(500).json({ message: "Erro interno do servidor" });
    }
  });
