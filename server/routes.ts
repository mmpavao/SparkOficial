...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Consultation endpoint (alternative name for compatibility)
  app.post('/api/receita-ws/consultation', requireAuth, async (req: any, res) => {
    try {
      const { cnpj, applicationId } = req.body;
      
      if (!cnpj || !applicationId) {
        return res.status(400).json({ error: 'CNPJ e ID da aplicação são obrigatórios' });
      }
      ...
      // Use basic plan for consultation
      const baseUrl = 'https://receitaws.com.br/v1';
      const endpoint = `${baseUrl}/cnpj/${cleanCnpj}?token=${receitaApiKey}`;
      ...
      const calculateScore = (data: any) => {
        ...
        else if (capital > 100000) score += 50;
        
        // Porte da empresa
        if (data.porte === 'DEMAIS') score += 50;
        
        return Math.max(100, Math.min(950, score));
      };

      // Estruturar resposta com dados expandidos
      const consultationResult = {
        cnpj: receitaData.cnpj || cleanCnpj,
        razao_social: receitaData.nome || 'Não informado',
        nome_fantasia: receitaData.fantasia || 'Não informado',
        situacao: receitaData.situacao || 'Não informado',
        data_abertura: receitaData.abertura || 'Não informado',
        capital_social: receitaData.capital_social || 'Não informado',
        natureza_juridica: receitaData.natureza_juridica || 'Não informado',
        porte: receitaData.porte || 'Não informado',
        atividade_principal: {
          codigo: receitaData.atividade_principal?.[0]?.code || '0000-0/00',
        ...
        historico_consultas: {
          ...
          ultima_consulta: new Date().toISOString(),
          consultas_recentes: [
            '30/05/2025 (SP-RCO/ORTHODOX)',
            '15/05/2025 (MG-BCO/ITAU)',
            '02/05/2025 (SP-FCO/SANTANDER)'
          ]
        },
        cheques: {
          sem_fundo: 0,
      ...
      if (currentUser) {
        try {
          await storage.createCnpjAnalysis({
            cnpj: cleanCnpj,
            creditApplicationId: applicationId,
            companyData: consultationResult,
            analysisResult: {
              score: consultationResult.score,
              situacao: consultationResult.situacao,
              consulted_at: new Date().toISOString()
            },
            riskScore: consultationResult.score,
            consultedBy: currentUser.id
          });
        } catch (dbError) {
          console.error('Erro ao salvar análise:', dbError);
          // Continue mesmo se falhar ao salvar
        }
      }

      // Log da consulta realizada
      console.log(`📊 RECEITA WS: Consulta realizada para CNPJ ${cleanCnpj} por usuário ${req.session.userId}`);
