...
import { 
  ...
  determineRiskLevel, 
  generateNeutralCreditData 
} from "./direct-data-integration";
...
<body>
  ...
  // Credit Score endpoint (POST - admin only) - Enhanced with Direct.data
  app.post('/api/credit/applications/:id/credit-score', requireAuth, async (req: any, res) => {
    try {
      ...
      console.log('🔍 Starting Direct.data company analysis for CNPJ:', cleanCnpj);
      
      // Get comprehensive company data from Direct.data API
      const directDataResponse = await callDirectDataAPI(cleanCnpj);
      
      if (!directDataResponse) {
        console.log('⚠️ Direct.data API call failed, using neutral analysis');
        const neutralScore = generateNeutralCreditData(cleanCnpj);
        
        const neutralCreditScoreData = {
          creditApplicationId: applicationId,
          cnpj: application.cnpj,
          creditScore: neutralScore.bankingScore,
          scoreDate: new Date(),
          hasDebts: false,
          hasProtests: false,
          hasBankruptcy: false,
          ...
          companyData: {
            cnpj: cleanCnpj,
            razaoSocial: 'Análise Indisponível',
            situacao: 'Dados não localizados',
            dataAbertura: null,
            capitalSocial: 'Não informado',
            porte: 'Não informado'
          }
        };
        
        const savedScore = await storage.createCreditScore(neutralCreditScoreData);
        return res.json(savedScore);
      }

      // Calculate enhanced credit score using Direct.data
      const enhancedCreditScore = calculateEnhancedCreditScore(directDataResponse);
      const creditRating = determineCreditRating(enhancedCreditScore);
      const riskLevel = determineRiskLevel(enhancedCreditScore);

      console.log('✅ Direct.data analysis complete:', {
        score: enhancedCreditScore,
        rating: creditRating,
        risk: riskLevel
      });

      // Create comprehensive credit score data using Direct.data
      const creditScoreData = {
        creditApplicationId: applicationId,
        cnpj: application.cnpj,
        creditScore: enhancedCreditScore,
        scoreDate: new Date(),
        hasDebts: false, // Will be enhanced when debt APIs are integrated
        hasProtests: false,
        hasBankruptcy: false, 
        hasLawsuits: false,
        
        // Comprehensive company data from Direct.data
        companyData: {
          cnpj: directDataResponse.cnpj,
          razaoSocial: directDataResponse.razaoSocial || 'Não informado',
          nomeFantasia: directDataResponse.nomeFantasia || directDataResponse.razaoSocial || 'Não informado',
          situacao: directDataResponse.situacaoCadastral || 'Não informado',
          dataAbertura: directDataResponse.dataFundacao || null,
          porte: directDataResponse.porte || 'Não informado',
          
          // Business classification
          naturezaJuridica: directDataResponse.naturezaJuridica?.descricao || 'Não informado',
          cnaeprincipal: directDataResponse.cnae?.principal?.descricao || 'Não informado',
          ramo: directDataResponse.operacionalData?.ramo || 'Não informado',
          
          // Operational data
          quantidadeFuncionarios: directDataResponse.operacionalData?.quantidadeFuncionarios || 0,
          faixaFuncionarios: directDataResponse.operacionalData?.faixaFuncionarios || 'Não informado',
          quantidadeFiliais: directDataResponse.operacionalData?.quantidadeFiliais || 'Não informado',
          matriz: directDataResponse.operacionalData?.matriz || true,
          
          // Financial indicators
          faixaFaturamento: directDataResponse.financialData?.faixaFaturamento || 'Não informado',
          faturamentoPresumido: directDataResponse.financialData?.faturamentoPresumido || 'Não informado',
          tributacao: directDataResponse.financialData?.tributacao || 'Não informado',
          opcaoSimples: directDataResponse.financialData?.opcaoSimples || 'Não informado',
          opcaoMEI: directDataResponse.financialData?.opcaoMEI || 'Não informado',
          
          // Contact information
          enderecos: directDataResponse.contatos?.enderecos || [],
          telefones: directDataResponse.contatos?.telefones || [],
          emails: directDataResponse.contatos?.emails || [],
          
          // Partners
          socios: directDataResponse.socios || [],
          
          // Enhanced metrics
          creditRating: creditRating,
          riskLevel: riskLevel,
          analysisSource: 'DIRECT_DATA',
          lastUpdate: directDataResponse.ultimaAtualizacao || new Date().toISOString()
        }
      };

      // Save credit score to database
      console.log('💾 Saving Direct.data credit analysis to database');
      const savedScore = await storage.createCreditScore(creditScoreData);
      
      console.log('✅ Credit score analysis completed and saved:', {
        applicationId,
        score: enhancedCreditScore,
        rating: creditRating,
        source: 'DIRECT_DATA'
