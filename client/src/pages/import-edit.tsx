...
export default function ImportEditPage() {
  ...
  // Populate form when import data loads
  useEffect(() => {
    if (importData) {
      // Check if import can be edited
      if (importData.status !== 'planejamento') {
        toast({
          title: "Erro",
          description: "Só é possível editar importações em planejamento.",
          variant: "destructive",
        });
        setLocation('/imports');
        return;
      ...
      if (importData.products) {
        try {
          ...
          const formattedProducts = productsData.map((product: any, index: number) => ({
            ...
            totalValue: Number(product.totalValue) || (Number(product.quantity) * Number(product.unitPrice)),
  ...
  }, [importData, form, toast, setLocation]);
  ...
  const updateImportMutation = useMutation({
    mutationFn: async (data: ImportFormData & { products: Product[] }) => {
      return await apiRequest(`/api/imports/${id}`, 'PUT', data);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['/api/imports'] });
      queryClient.invalidateQueries({ queryKey: ['/api/imports', id] });
      toast({
        title: "Importação atualizada",
        description: "A importação foi atualizada com sucesso.",
      });
      setLocation(`/imports/${id}`);
    },
    onError: (error: any) => {
      toast({
        title: "Erro",
  ...
  const onSubmit = (data: ImportFormData) => {
    ...
    updateImportMutation.mutate({ ...data, products });
  };
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Button 
          variant="ghost" 
          onClick={() => setLocation(`/imports/${id}`)}
          className="p-2"
        >
          <ArrowLeft className="w-4 h-4" />
        </Button>
        <div>
          ...
          <p className="text-gray-600">{importData.importName} - {importData.importNumber}</p>
        </div>
      </div>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button 
                  ...
                  disabled={updateImportMutation.isPending || products.length === 0}
                >
