...
export const zhTranslations: Translations = {
  ...
  admin: {
    ...
    company: '公司',
    requestDate: '申请日期',
    overview:The code has been modified to ensure Portuguese is the default language, resetting any stored language that isn't Portuguese. '概览',
    applications: '申请',
    users: '用户',
    imports: '进口',
    name: '姓名',
...
export const getCurrentLanguage = (): Language => {
  // Sempre retornar português como padrão inicial
  if (typeof window === 'undefined') return 'pt';

  // Limpar qualquer idioma armazenado que não seja português para resetar
  const stored = localStorage.getItem('spark-comex-language') as Language;
  if (stored && stored !== 'pt') {
    localStorage.setItem('spark-comex-language', 'pt');
    return 'pt';
  }

  return 'pt'; // Sempre português por padrão
};
